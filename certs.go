package openhue

// HueBridgeRootCAs contains the PEM-encoded root CA certificates used by Philips Hue Bridges.
// These certificates are used to verify the TLS connection to the bridge.
//
// Source: https://developers.meethue.com/develop/application-design-guidance/using-https/
const HueBridgeRootCAs = `-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----
-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----
`
